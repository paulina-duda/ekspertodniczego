...
Three biomorphs, two of them alive.

The sibling set in [`../source/`](../source/) runs two mathematical processes
against one biological one — Gray-Scott and differential growth against a slime
mould. This set inverts the ratio. **Hyphae** and **Cleavage** are processes a
...
**Hyphae.** A hypha only ever extends at its tip. It wanders, it branches, and
...
**Cleavage.** The one kind of growth that does not grow. A fertilised egg
...
**Sandpile.** A cell holding four grains gives one to each neighbour. That is
