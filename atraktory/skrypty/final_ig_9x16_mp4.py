#!/usr/bin/env python3
"""Generate final 9:16 Instagram MP4 variants with low-blue palettes."""
...
def build_palette(palette_name: str, background_name: str) -> list[tuple[int, int, int]]:
...
def choose_h264_encoder(ffmpeg: str, bitrate: str, maxrate: str) -> list[str]:
...
def write_mp4(
    ...
    encoder_args = choose_h264_encoder(ffmpeg, args.bitrate, args.maxrate)
    command = [
        ffmpeg,
        "-y",
        "-f",
    ...
    stream_frames(process, points, palette_array, args)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create final low-blue 9:16 Lorenz MP4 files.")
    parser.add_argument("-o", "--output-dir", type=Path, default=ROOT_DIR / "final")
    parser.add_argument("--palette", choices=sorted(FINAL_PALETTE_STOPS), default="ember_garden")
    parser.add_argument("--background", choices=sorted(FINAL_BACKGROUNDS), default="black")
...
def main() -> None:
    ...
    presets = FINAL_PRESETS if args.all_presets else [(args.palette, args.background)]

    for palette_name, background_name in presets:
        output = args.output_dir / (
            f"lorenz_{palette_name}_{background_name}_"
            f"{args.width}x{args.height}_{args.duration:.0f}s_{args.fps}fps.mp4"
        )
        write_mp4(output, points, build_palette(palette_name, background_name), args)
