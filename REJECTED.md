...
| `packing` | a bacterial monolayer running out of plane, cells standing up rather than spreading (Beroz 2018) | **T4** | built, measured clean and rejected on the finished cut — Paulina's call, and the second way T4 fails: you can say what it is and it is not worth looking at. The `--tag orientation` variant goes with it |
| `plaque` | phage clearing a bacterial lawn, with the resistant cells already in it (Luria & Delbrück 1943) | **T4** | same call on the same day as `packing`: nothing wrong with the process or the numbers, the picture is simply ugly |
...
| `cascade` | Bak-Sneppen coevolution: the least-fit site and its neighbours replaced, avalanches with no external cause | **T1** | the threshold is a relaxation — 73.4% of its rise in the first quarter, 3.6% in the last — and at criticality the avalanche is flat (0.0058 / 0.0054 / 0.0062 / 0.0056 / 0.0065); the only scalar that accumulates is cumulative coverage, a reveal, and the field is frozen at **0.56–0.61% live** against `ridge`'s rejected 1.6% |
| `quine` | BFF: self-replicators emerging from a soup of random programs, no fitness function anywhere | **T4** | a soup of tapes has no picture — the byte field is uniform grey static at 200 px and a descent field is white dots on black, which is `culture` and `cortex` again; killed on the still before the 75-minute emergence run was worth starting |
| `upkeep` | autopoiesis (Varela, Maturana & Uribe 1974): the SCL chemistry, membranes continuously rebuilt from inside | **T4** | T1 passes with a catalyst clock (0 / 7 / 12 / 17 / 28 cells, 25.0% first quarter, 39.3% last, 44.5% live) and the enclosure never becomes visible — an SCL membrane closes at the diffusion length around a point catalyst, so the interior is 2–7 sites, 16 px across at the count the profile needs and 32 px at half the count; scattered blobs either way |
